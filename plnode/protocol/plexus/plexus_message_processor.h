...
#include "../../message/p2p/MessageRetrieveReply.h"
...
public:
	...
	bool processMessage(ABSMessage* message)
	{
		...
		if (forward)
		{
			...
			{
				if (message->getMessageType() == MSG_PLEXUS_GET_REPLY
						|| message->getMessageType() == MSG_PLEXUS_PUT_REPLY)
				{
					OverlayID id(message->getSrcOid().GetOverlay_id(),
							message->getSrcOid().GetPrefix_length(),
							message->getSrcOid().MAX_LENGTH);
					HostAddress ha(message->getSourceHost(),
		...
		if (message->getMessageType() == MSG_PLEXUS_PUT)
		{
			...
			reply = new MessagePUT_REPLY(container_peer->getHostName(),
					...
					putMsg->getSourceHost(), putMsg->getSourcePort(),
					container_peer->getOverlayID(), OverlayID(), SUCCESS,
					putMsg->getSrcOid(), putMsg->GetDeviceName());
		...
		else if (message->getMessageType() == MSG_PLEXUS_GET)
		{
			...
			if (index_table->lookup(msg->GetDeviceName(), &hostAddress))
			{
				...
				MessageGET_REPLY *reply = new MessageGET_REPLY(
						...
						container_peer->getOverlayID(), OverlayID(), SUCCESS,
						msg->getSrcOid(), hostAddress, msg->GetDeviceName());
			...
			} else
			{
				...
				MessageGET_REPLY *reply = new MessageGET_REPLY(
						...
						container_peer->getListenPortNumber(),
						msg->getSourceHost(), msg->getSourcePort(),
						container_peer->getOverlayID(), OverlayID(),
						ERROR_GET_FAILED, msg->getSrcOid(), hostAddress,
						msg->GetDeviceName());
		...
		else if (message->getMessageType() == MSG_PLEXUS_GET_REPLY)
		{
			...
			if (exists)
			{
				...
				RetrieveMessage* rtr_msg = new RetrieveMessage(
						container_peer->getHostName(),
						container_peer->getListenPortNumber(),
						msg->getHostAddress().GetHostName(),
						msg->getHostAddress().GetHostPort(),
			...
			LogEntry *entry = new LogEntry(LOG_GET, key.c_str(), "iidssi",
					msg->getResolutionHops(), ip_hops, latency, status.c_str(),
			...
			//cache->print();
		} else if (message->getMessageType() == MSG_RETRIEVE)
		{
			...
			MessageRetrieveReply* rtr_reply = new MessageRetrieveReply(
					...
					msg->getSourceHost(), msg->getSourcePort(),
					container_peer->getOverlayID(), msg->getSrcOid(), SUCCESS,
					result, msg->GetDeviceName());

			rtr_reply->setOriginSeqNo(msg->getSequenceNo());
			plexus->addToOutgoingQueue(rtr_reply);
		} else if (message->getMessageType() == MSG_RETRIEVE_REPLY)
		{
			MessageRetrieveReply *msg = ((MessageRetrieveReply*) message);
			int hash_name_to_get = (int) urlHash(msg->getDeviceName().c_str())
					& 0x003FFFFF;
...
//				plexus->getUnresolvedGet().add(ind, r);
		...
		} else if (message->getMessageType() == MSG_PLEXUS_PUT_REPLY)
		{
			...
			int hash_name_to_publish = (int) urlHash(
					msg->getDeviceName().c_str()) & 0x003FFFFF;
			...
			LogEntry *entry = new LogEntry(LOG_PUT, key.c_str(), "iidsi",
					msg->getResolutionHops(), ip_hops, latency,
					msg->getDeviceName().c_str(),
					msg->getSrcOid().GetOverlay_id());
		...
		else if (message->getMessageType() == MSG_PEER_INIT)
		{
			...
			container_protocol->setIndexTable(
					new LookupTable<string, HostAddress>());
			container_protocol->setCache(
					new Cache(new CacheInsertEndpoint(), new CacheReplaceLRU(),
							container_protocol->getRoutingTable(),
							plexus->getProactiveCache(),
			...
			if (strcmp(container_peer->getCacheType().c_str(), "proactive")
					== 0)
			{
				...
				while (rtable_iterator.hasMoreKey())
				{
					...
					HostAddress dst_ha;
					container_protocol->getRoutingTable()->lookup(dst_oid,
							&dst_ha);
					MessageCacheMe *msg = new MessageCacheMe(
							container_peer->getHostName(),
							container_peer->getListenPortNumber(),
							dst_ha.GetHostName(), dst_ha.GetHostPort(),
							container_peer->getOverlayID(), dst_oid);
					msg->setOverlayTtl(2);
		...
		{
			PeerDynChangeStatusMessage* dcsMsg =
					(PeerDynChangeStatusMessage*) message;
		...
		} else if (message->getMessageType() == MSG_CACHE_ME)
		{
					...
					== 0)
			{
				...
				HostAddress ha(cache_msg->getSourceHost(),
						cache_msg->getSourcePort());
				HostAddress dummy;
				//cache->add(oid, ha);
				if (!(oid == container_peer->getOverlayID())
						&& !plexus->getRoutingTable()->lookup(oid, &dummy))
				...
				if (message->getOverlayTtl() > 0)
				{
					...
					while (rtable_iterator.hasMoreKey())
					{
						...
						MessageCacheMe *msg = new MessageCacheMe(
								...
								message->getSourcePort(), dst_ha.GetHostName(),
								dst_ha.GetHostPort(), message->getSrcOid(),
								dst_oid);
